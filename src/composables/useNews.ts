...
export const useNews = () => {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      // Firebase Storageから最新データを取得
      const response = await $fetch('https://storage.googleapis.com/oki-ferryguide.appspot.com/data/news.json')
      if (response && Array.isArray(response)) {
        newsList.value = response
        // キャッシュに保存
        await setCachedNews(response)
      }
    } catch (err) {
