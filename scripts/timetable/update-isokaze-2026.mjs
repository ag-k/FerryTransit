...
const DEFAULT_SOURCE_FILE = join(ROOT, 'gtfs', 'raw', 'ferry', 'oki-kanko-isokaze-2026.json')
const DEFAULT_TIMETABLE_FILE = join(ROOT, 'timetable.json')
const MANAGED_NAME = 'ISOKAZE'
...
export const validateIsokazeSource = data => {
  ...
  const expectedAfterCoverage = addOneDay(source.coverage_end)
  if (expectedStart !== expectedAfterCoverage) {
...
export const buildIsokazeTrips = (data, firstTripId) => {
  validateIsokazeSource(data)
  if (firstTripId !== undefined && (!Number.isInteger(firstTripId) || firstTripId <= 0)) {
    throw new Error(`firstTripIdが不正です: ${firstTripId}`)
  }
  let overrideTripId = firstTripId
  const trips = []

  for (const schedule of data.schedules) {
    const legs = data.patterns[schedule.pattern_id].legs
    let tripId = overrideTripId ?? schedule.first_trip_id
    ...
    if (overrideTripId !== undefined) {
      overrideTripId = tripId
    }
  }
...
export const updateIsokazeTimetable = (currentTrips, sourceData) => {
  validateIsokazeSource(sourceData)
  ...
  const added = buildIsokazeTrips(sourceData)
...
const main = () => {
  ...
  const result = updateIsokazeTimetable(currentTrips, sourceData)
