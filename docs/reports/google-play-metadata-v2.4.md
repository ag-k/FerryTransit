...
- アプリアイコンは登録済みだが、「新しい仕様に従っていません」という警告が表示される。2026-07-23に下記の512pxアイコンへ差し替えて未公開保存し、再読み込み後も新しいファイルが選択されていることを確認したが、警告は消えなかった。画像の寸法・容量・形式だけが原因ではないため、v2.4提出前にPlay Console側の判定理由と新仕様への適合方法を継続確認する。
- ローカルの512pxアイコンを内容変更なしで`output/google-play-assets/app-icon-512.png`へ準備した。512×512、8-bit RGBA PNG、62,986 bytes、角丸・外部シャドウなし、SHA-256 `dea3cf6c28a86e80f86b3d261899ac5d89b29baaf00c6cb386b821022a4e771a`。Play Consoleへ登録し、動的マスクのプレビューとファイル選択状態は確認済み。残る新仕様警告は別途解消する。
