...
- 公開中の旧アプリアイコンには「新しい仕様に従っていません」という警告が表示されていた。詳細表示では、英語（米国）の新仕様アイコンがまだないことが原因と明記されていた。2026-07-23に下記の512pxアイコンを素材ライブラリから英語（米国）の掲載欄へ追加し、旧アイコンを削除して`1/1`の状態で未公開保存した。再読み込み後に警告が消えたことを確認した。
- ローカルの512pxアイコンを内容変更なしで`output/google-play-assets/app-icon-512.png`へ準備した。512×512、8-bit RGBA PNG、62,986 bytes、角丸・外部シャドウなし、SHA-256 `dea3cf6c28a86e80f86b3d261899ac5d89b29baaf00c6cb386b821022a4e771a`。Play Consoleへの登録、動的マスクのプレビュー、新仕様警告の消失まで確認済み。
- 携帯電話スクリーンショットは、公開中の日英各2枚をv2.4用の日英各4枚（1080×2220）へ置換して未公開保存した。両言語で`4/8`、新しい4ファイルだけが選択されていることを確認した。
- 7インチ・10インチタブレットには、Pixel Tablet / Android 14の横向き日英各5枚（2560×1600）をそれぞれ登録して未公開保存した。両言語・両欄で`5/8`となり、7インチ欄でも同画像が寸法エラーなく受理された。
- Chrome拡張からのローカルファイル選択が許可されなかったため、Google Driveの専用フォルダ[`FerryTransit v2.4 Play Assets`](https://drive.google.com/drive/folders/1ll5SI6uDaHpAqmIq4mrUg3bTq9K13wZx)へ、アイコン1枚、携帯電話画像8枚、タブレット画像10枚の計19 PNGを言語・端末種別付きの名前でアップロードした。Drive APIで全19件の`image/png`、ファイル名、非0サイズを読み戻し、欠落0件を確認した。フォルダ所有者が`corp@oki-digilab.com`、Play Console操作アカウントが`ag.aeg.age@gmail.com`で、そのままではアクセス拒否となることを確認したため、画像19件だけを後者へ読み取り共有した。Play Consoleの「共有アイテム」からアイコン1枚、携帯電話画像8枚、タブレット画像10枚を選択し、日英の掲載情報へ反映して未公開保存した。審査送信・公開は行っていない。
...
- Play Consoleのデータセーフティ「プレビュー」を再確認し、「データ収集は申告されていません」「第三者と共有されるデータはありません」と表示されること、プライバシーポリシーURLが`https://transit.oki-digilab.com/privacy`であることを確認した。ステップ2の実回答も「対象になる種類のユーザーデータを収集または共有しますか？」が「いいえ」のままであり、上記の実装・ポリシーとの不整合を画面上で再現した。回答は変更・保存していない。
- 2026-07-25にユーザーの明示承認を受け、下記案をPlay Consoleの未公開変更として保存した。プレビューは「アプリのアクティビティ」として「アプリのインタラクション数」「アプリ内の検索履歴」を収集、第三者共有なし、送信中の暗号化あり、削除リクエスト方法なし、プライバシーポリシー`https://transit.oki-digilab.com/privacy`を表示した。保存後に「変更を保存しました。[公開の概要]で審査に送信してください」を確認し、審査送信は「後で行う」として保留した。公開中の回答はv2.4提出・審査完了まで旧内容のままとなる。
...
実装とポリシーに対する保守的な回答として、2026-07-25にPlay Consoleへ未公開保存した内容は次のとおり。
...
- 転送時暗号化: `はい`
- アカウント作成・外部アカウントログイン: `なし`
- 削除リクエスト方法: `提供なし`（個人に関連付けたデータを保持せず、個別データを特定できない）

Cloud Firestore SDKが自動収集するFirebase User Agent（OSバージョン、端末名・モデル等）はFirebaseの説明上ユーザー・端末識別子へリンクされない。最新AABのFirebase構成と独自集計実装を照合し、追加データ種別の申告は不要と判断した。
...
- [x] データセーフティのプライバシーポリシーURLが`https://transit.oki-digilab.com/privacy`であることをプレビューで確認する。
- [ ] 連絡先メール`koyama@naturebot-lab.com`が現在の運用窓口として有効か、リリース責任者が確認する。
