...
- iOS: Simulator Release、端末向けRelease、Archive、App Store用IPA生成に成功。IPA SHA-256は`556d7d2984e734b0ed0f7e28a55b558c7fae97152472b40c35a037a9db0866e4`。Apple Distribution署名、App Storeプロファイル、`get-task-allow=false`、時刻表・GTFS・bus-searchデータ非同梱を確認した。2026-08-03 20:40 JSTにApp Store Connectへのアップロードが成功し、20:42 JSTの処理完了通知と20:43 JSTのTestFlight配信可能通知を確認した。2026-08-04 13:26 JSTに日英リリースノート、iPhone / iPadの日英スクリーンショット各3枚、build 25004、既存評価維持、承認後の自動リリース、段階的リリースなしを確認してApp Reviewへ提出した。提出IDは`18215a56-f472-4f6e-b557-757ff7b6c477`、状態は「審査待ち」。
- Android: production同期、`bundleRelease`、Release lint、署名、bundletool 1.18.3検証に成功。AABは`output/releases/ferrytransit-v2.5-25004-bbecab9.aab`、SHA-256は`d4686486b0b9d205dd618252aee3bb5c7df69b3453bad23416de4be1b7d85f09`。Application ID、v2.5 / 25004、minSdk 23 / targetSdk 36、Play登録証明書、禁止データ非同梱を確認し、2026-08-03 20:56 JSTにGoogle Play内部テスターへ公開した。2026-08-04 15:48 JSTに同じAABを製品版リリース`25004 (2.5)`へ追加し、日英リリースノート、対象デバイス減少0件、全対象国（177か国・地域）、公開率100%、管理対象の公開オフを確認してGoogle審査へ送信した。Play Consoleでは「審査中の変更」、説明は「完全公開を開始」と表示された。R8難読化解除ファイル未添付の警告は、minify無効のため非阻止と判断した。
...
### Android Google Play内部テスト build 25004実機QA（2026-08-04 00:01–00:28 JST）

- 対象: moto g05 / Android 15。Google Playの内部テスト画面で「更新」を実行し、versionCode `25003`から`25004`へデータを削除せず更新した。versionName `2.5`、installer `com.android.vending`、初回インストール`2026-07-24 17:21:54`を維持し、最終更新だけ`2026-08-04 00:01:22`へ変わった。
- 更新保持: 起動直後に更新前の出雲空港→西郷・2026-08-04条件を表示した。8月2日の出雲→西郷、伊丹→西郷、境港駅⇔西郷、七類港→境港駅などの検索履歴、日本語、システムテーマ、地図表示設定を確認した。更新時点のお気に入りは0件で、既存お気に入りの保持は検証できなかった。
- 恒久修正: 船の別府→菱浦、2026-08-09を検索し、いそかぜ`08:25→08:32`、`09:17→09:24`、`10:02→10:09`とフェリーどうぜん各便を表示した。運航状況画面で明示更新して時刻表へ戻った後も同じ予定便を表示し、運航状況更新で予定便が削除されないことを確認した。
- 運航状況の実データは隠岐汽船フェリー・島前内航船とも「運航状況情報がありません」で、臨時便が存在しなかった。臨時便が予定便へ追加表示される実機確認は継続する。
- JAL: 更新前履歴から伊丹→西郷を復元し、JAL2331 `12:15→13:05`、隠岐空港15分乗換、空港連絡バス、西郷`13:30`、合計`¥520 + 航空運賃（変動）`を確認した。出雲→西郷もJAL3433 `09:00→09:30`、15分乗換、西郷`09:55`、同じ変動運賃表示を確認した。
- 外部リンク: 伊丹・出雲の両結果から「JALで運賃を確認」を開き、Capacitor Browser Controllerを経由してChrome Custom Tabの`https://www.jal.co.jp/domestic/ja-jp/flights-from-oki`へ遷移した。
- はつみ交通: 七類港→境港駅は`10:05→10:20`、`18:10→18:25`、境港駅→七類港は`08:24→08:39`、`13:25→13:40`、`16:07→16:22`を表示し、全便15分・500円を確認した。境港駅→西郷は、はつみ交通`16:07→16:22`（500円）から七類港でレインボージェットへ乗り継ぎ、合計7,860円。西郷→境港駅はフェリーおきから七類港ではつみ交通`18:10→18:25`（500円）へ乗り継ぎ、合計4,370円を確認した。
- 復帰: JAL結果表示中にホームへ移動して復帰し、出雲→西郷の結果を保持した。force-stop後のコールド起動でも更新前からの出雲空港→西郷条件、日付、履歴、設定が残った。
- ログ: 起動クラッシュ、ANR、SSLHandshakeException、Capacitorの致命的例外は0件。Android WebViewのpage-load metricsがfirst paintの順序を報告する既知の非致命的ログが出たが、画面描画・操作・検索には影響しなかった。
- 未完了: 接続がワイヤレスADBのみのため、Wi-Fiを切断すると操作・証跡取得ができない。完全オフライン→通信復帰はUSBデバッグ接続後に実施する。実在する運航状況臨時便と、更新前に登録済みのお気に入りの保持確認も継続する。
- 判定: Android build 25004は部分合格。恒久修正の予定便保持と主要JAL経路は合格したが、REL-25-07は継続する。

### iOS TestFlight build 25004実機QA（2026-08-04 00:47–01:09 / 06:26–10:46 JST）

- 対象: ePhone / iOS 26.5.2。TestFlightからデータを削除せずbuild `25003`→`25004`へ更新し、CoreDeviceのインストール済みアプリ情報でversion `2.5` / bundle version `25004`を確認した。
- 恒久修正: 船の別府→菱浦、2026-08-09を検索し、レインボージェット`07:40→07:50`、フェリーどうぜん`07:50→08:02`、いそかぜ`08:25→08:32`、フェリーどうぜん`08:40→08:52`を実画面で確認した。
- 運航状況: 明示更新を実行した。隠岐汽船フェリー・島前内航船とも「運航状況情報がありません」で、実在する臨時便がなかったため、臨時便追加表示は継続確認とする。
- JAL: 更新前履歴から伊丹→西郷を復元し、JAL2331 `12:15→13:05`、15分乗換、空港連絡バス、バス運賃520円、合計`¥520 + 航空運賃（変動）`を確認した。出雲→西郷もJAL3433 `09:00→09:30`、15分乗換、空港連絡バス、520円、同じ変動運賃表示を確認した。
- 外部リンク: 伊丹・出雲の両経路からアプリ内ブラウザーで`jal.co.jp`のJAPAN AIRLINES公式「隠岐空港発のおトクな航空券」ページへ遷移した。
- 更新保持: 更新前からの8月1日の検索履歴、日本語、ダークテーマ、別府→菱浦のお気に入りルートを保持した。履歴から伊丹・出雲JAL経路を再検索し、お気に入りから8月9日のいそかぜを再検索した。
- ログ: build 25004の起動時に`KeyboardPlugin: resize mode - native`を出力し、クラッシュ、SSL、Capacitorの致命的例外はなかった。ログ回収操作によるsignal 15終了は試験ツールがアプリを停止したもの。
- 復帰: ホーム画面へ移動し、Appスイッチャーから直ちに復帰して検索条件・画面状態を保持した。CoreDeviceでプロセスを強制終了してログ取得付きで再起動し、更新前から保持された出雲空港→西郷・2026-08-04条件で正常起動した。
- 完全通信遮断: Xcode Device ConditionsのNetwork Link / `100% packet loss`（上下0 Mbps、packet loss 100%）を開始し、Condition activeを確認した。通信条件はiPhoneミラーリングにも適用されて画面接続が切れたが、USB接続は維持した。事前の機内モード＋Wi-Fiオフ試験を含め、USB経由でbuild 25004を起動でき、致命的ログ・異常終了はなかった。
- 通信復帰: Xcode上でNetwork Link条件を停止し、iPhoneの通常通信設定を変更せずミラーリングを再接続した。アプリは同じ検索条件で表示を継続し、運航状況の更新`8/4 06:30`、フェリーおき・レインボージェットの定期運航、8月9日別府→菱浦のいそかぜ`08:25→08:32`とフェリーどうぜんを再取得・再表示した。
- はつみ交通: 更新前履歴から境港駅→西郷を復元し、はつみ交通`16:07→16:22`（500円）→七類港で徒歩3分→レインボージェット`16:50→17:59`（7,360円）、合計7,860円を確認した。逆方向は西郷`15:40`→フェリーおき→七類港`18:05`→徒歩3分→はつみ交通`18:10→18:25`（500円）、合計4,370円を確認した。build 25003で確認済みの七類港―境港駅直行便とあわせ、build 25004でもはつみ交通の両方向・500円運賃・船との双方向乗換に退行がないことを確認した。
- 未完了: 実在する運航状況臨時便が現在データになく、臨時便追加表示は未確認。
- 判定: iOS build 25004は主要恒久修正、はつみ交通、JAL経路、更新保持まで合格。実在臨時便の確認待ちのためREL-25-07は継続する。

### 実在臨時便の履歴再生による代替検証（2026-08-04 11:17 JST）

- 現行API: `GET https://ship.nkk-oki.com/api/status`を再取得し、2026-08-04のいそかぜ・フェリーどうぜんはともに`extraShips: []`であることを確認した。本番運航状況へ検証用の臨時便を一時登録すると利用者へ誤情報を配信するため実施しない。
- 実在履歴: 運航状況本番DBを読み取り専用で照合し、直近の臨時便はstate `5199`、2025-04-13、いそかぜ、別府→菱浦`19:08`、菱浦→来居`19:18`、来居→菱浦`19:40`、菱浦→別府`20:02`の4区間だったことを確認した。
- 同一性: 恒久修正版SHA `bbecab9b9b001841d118f0090b48c77297bda429`と現在の`src/stores/ferry.ts`、`src/pages/status.vue`および対象試験に差分がないことを`git diff --quiet`で確認した。
- 回帰試験: `npm run test -- src/test/unit/stores/ferry.test.ts src/test/unit/pages/status.test.ts`を実行し、2ファイル・34件が合格した。API仕様形式の`extraShips`マッピング、臨時便ID `1000`からの追加、再取得時の重複防止と置換、公式予定便の保持、オフラインキャッシュ復元後の再適用、いそかぜ・フェリーどうぜんの臨時便表表示を含む。
- 実機側の補完証跡: build 25004はiOS / Androidとも同一production APIからのオンライン取得、予定便保持、通信復帰を確認済み。iOSは完全通信遮断・コールド起動、Androidはコールド起動も合格している。
- 制約と判定: ストア配布済みバイナリへ履歴レスポンスを安全に注入する経路はなく、実在臨時便の自然発生を待つ実機確認だけが未実施である。2026-08-04 11:20 JSTに依頼者（Codexユーザー）がQA責任者として履歴再生をN/A代替証跡に承認し、REL-25-07を解決済みとした。
...
### Google Play内部テスト版 Android実機QA（2026-08-02 16:14–17:17 JST）

- 対象: `moto g05`（Android 15 / API 35）。Androidのワイヤレスデバッグでペア設定し、ADB接続を確立した。
- `com.naturebotlab.ferrytransit`のインストール情報からv2.5（versionCode 25003）、初回インストール`2026-07-24 17:21:54`、最終更新`2026-08-02 15:22:40`を確認した。Google Play内部テスト版の更新インストール後であることを端末情報から独立確認した。
- アプリをホット起動し、時刻表の船・バス・飛行機タブ、検索欄、地図、下部ナビゲーションが表示され、白画面、クラッシュ、主要レイアウト崩れがないことを確認した。
- バスの出発地選択ダイアログは「バス停」だけを表示し、七類港、境港駅、松江駅などの各候補にバスアイコンが表示された。港・空港・フェリー単独候補の混入は確認されなかった。
- 七類港→境港駅は、はつみ交通の`10:05→10:20`、`18:10→18:25`の2便を表示した。境港駅→七類港は`08:24→08:39`、`13:25→13:40`、`16:07→16:22`の3便を表示した。いずれも事業者・路線名は「はつみ交通 隠岐汽船連絡バス（七類港⇔境港駅）」で、所要15分だった。
- QA中の対象プロセスLogcatにクラッシュ、ANR、未処理例外はなかった。Android System WebView / Chromiumのpage-load計測に関する`Invalid first_paint`が反復したが、画面描画・検索操作への影響は確認されなかった。
- 便詳細で運航会社「はつみ交通」、七類港・境港駅、大人片道`¥500`、掲載期間`2026/06/08–2026/12/31`を確認した。
- 船→バス: 西郷`15:40`→フェリーおき→七類`18:05`→徒歩3分→はつみ交通`18:10`→境港駅`18:25`、合計`¥4,370`（船`¥3,870` + バス`¥500`）を確認した。
- バス→船: 境港駅`16:07`→はつみ交通→七類`16:22`→徒歩3分→レインボージェット`16:50`→西郷`17:59`、合計`¥7,860`（バス`¥500` + 高速船`¥7,360`）を確認した。
- 伊丹→西郷はJAL2331 `12:15→13:05`、空港連絡バス`13:20→13:30`、`¥520 + 航空運賃（変動）`を表示した。出雲→西郷はJAL3433 `09:00→09:30`、空港連絡バス`09:45→09:55`、同じ変動運賃表示を確認した。
- 伊丹・出雲の両経路で「JALで運賃を確認」をタップし、Capacitor Browserから`jal.co.jp`のJAPAN AIRLINES「隠岐空港発のおトクな航空券」ページ表示まで確認した。
- パッケージ情報は初回インストール`2026-07-24 17:21:54`、最終更新`2026-08-02 15:22:40`で、アプリデータを削除しない更新状態だった。保持テスト用にダークテーマとお気に入り`出雲空港 → 西郷`を設定し、検索履歴を作成後、2回の強制終了・コールド起動を実施した。ダークテーマ、お気に入り、伊丹・出雲・はつみ交通・船乗換の履歴が再起動後も保持された。
- 対象プロセスLogcatにクラッシュ、ANR、未処理例外はなかった。Android System WebView / Chromiumのpage-load計測に関する`Invalid first_paint`、Motorola / MediaTekの任意ライブラリ`libmagtsync.so`未搭載、Google Play ServicesのPhenotypeフラグに関する端末側警告が出たが、画面描画・検索・保存データ・外部リンクへの影響は確認されなかった。Android実機ゲートは合格とする。
...
| REL-25-07 | 解決済み | 現行APIに実在臨時便がなく、自然発生時のbuild 25004実機表示を実施できなかった | 直近の実在臨時便（2025-04-13・いそかぜ4区間）と同一処理を覆う追加・置換・予定便保持・オフラインキャッシュ・画面表示の試験34件が合格 | 2026-08-04 11:20 JST、依頼者（Codexユーザー）がN/A代替証跡として承認 |
| REL-25-08 | 解決済み | 固定SHA `bbecab9`と同一アプリソースの本番Hosting公開・スモークQA完了 | 影響なし | 2026-08-03 23:20–23:25 JST確認済み |
| REL-25-04 | 解決済み | QA責任者、リリース責任者のGo承認を記録 | 影響なし | 2026-08-04 11:20 JST、依頼者（Codexユーザー）承認 |

従来build 25003ではWeb / Storage本番反映、iOS / Android実機、オフライン・復帰を含む主要QAが合格している。恒久修正版build 25004も全自動ゲート、iOS / Android Release成果物、両ストアへの配布登録、App Store Connect処理完了、同一アプリソースの本番Hosting公開後QA、はつみ交通を含む主要実機QAまで合格した。現在データに存在しない臨時便は実在履歴再生をN/A代替証跡としてQA責任者が承認し、2026-08-04 11:20 JSTにQA責任者・リリース責任者のGo承認を記録した。v2.5全体の判定をGoとする。
