...
### Google Play内部テスト版 Android実機QA（2026-08-02 16:14–16:20 JST）
...
- 未確認: 船との乗換、片道500円の詳細、伊丹・出雲JAL経路と外部リンク、設定・お気に入り・履歴の更新保持。これらを完了するまでAndroid実機ゲートは継続する。
...
| REL-25-02 | リリース阻止 | iOS build 25003の主要経路・保持QAとAndroid Release構成のオフラインQAを実施済み。Google Play内部テスト版v2.5（25003）をmoto g05 / Android 15で起動し、versionCode、バス停選択、はつみ交通の両方向検索、クラッシュなしを確認した | Android実機で船との乗換、500円運賃、伊丹・出雲JAL経路と外部リンク、更新保持が未確認 | 残るAndroid実機QAを完了する |
...
Web / Storage本番反映に重大不具合は確認していない。iOS TestFlight実機QAで確認したJAL外部リンク遷移不良と履歴の内部コード露出は、build 25003の実機再QAで解決確認した。モバイルSafari / Chromeの実機QA、Android Release構成による完全オフライン・通信復帰QA、Google Play内部テスト配布、Android実機の版番号・バス停選択・はつみ交通両方向検索も合格した。Android実機の残経路・保持QA、全プラットフォーム同一SHA、承認ゲートが未完了のため、v2.5全体の判定はNo-Go（確認継続）とする。
