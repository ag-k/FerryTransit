...
- 制約と判定: ストア配布済みバイナリへ履歴レスポンスを安全に注入する経路はなく、実在臨時便の自然発生を待つ実機確認だけが未実施である。履歴再生をN/A代替証跡として受け入れるかはQA責任者の承認事項とし、承認まではREL-25-07を継続する。
...
従来build 25003ではWeb / Storage本番反映、iOS / Android実機、オフライン・復帰を含む主要QAが合格している。恒久修正版build 25004も全自動ゲート、iOS / Android Release成果物、両ストアへの配布登録、App Store Connect処理完了、同一アプリソースの本番Hosting公開後QA、はつみ交通を含む主要実機QAまで合格した。一方、現在データに存在しない臨時便、全TODO確認、QA・リリース承認ゲートが未完了のため、v2.5全体の判定はNo-Go（確認継続）とする。
