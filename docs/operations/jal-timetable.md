...
1. JAL公式時刻表を取得・検証する
2. 隠岐空港連絡バスと公開時刻表を1回だけ生成する
3. 入力・生成JSONに変更がなければ、コミット・バックアップ・公開を省略する
4. 変更された入力・生成JSONを `dev` ブランチへコミット・pushする
5. そのコミットと同じ生成JSONを `oki-ferryguide-dev.firebasestorage.app/data/timetable.json` へバックアップ付きで公開し、SHA-256を検証する
...
```bash
npm run timetable:build:dry-run
