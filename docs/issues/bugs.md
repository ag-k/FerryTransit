...
#### リリース候補SHAでの最終確認（2026年7月25日）

リリース候補SHA `b4d7830`を固定後、通常Chromeのdev公開版で境港→菱浦（2026-07-05、08:00発）を検索した。履歴は再読み込み後も08:00を保持し、23:00への変換はなく、「再検索」後のURLクエリと入力欄も08:00だった。今回作成したQA履歴だけを削除し、既存履歴は保持した。console error/warningは0件だったため解決済みとする。
...
リリース候補SHA `b4d7830`を固定後、通常Chromeのdev公開版と公開時刻表1,100件で再確認した。西郷→菱浦（2026-07-05、18:00発）はレインボージェット18:05→18:36、西郷→別府は18:05→18:49がそれぞれ1件だけ表示され、いずれも乗換のない単一経路だった。console error/warningは0件だったため解決済みとする。
...
リリース候補SHA `b4d7830`を固定後、通常Chromeのdev公開版と公開時刻表1,100件で来居→菱浦（2026-07-05、08:00発）を再確認した。いそかぜ08:05→08:32の通し乗車が検索結果の先頭に表示され、欠落はなかった。console error/warningは0件だったため解決済みとする。
...
リリース候補SHA `b4d7830`を固定後、通常Chromeのdev公開版と公開時刻表1,100件で境港→菱浦（2026-07-05、08:00発）を再確認した。17:27着（別府・いそかぜ）、17:52着（別府・フェリーどうぜん）、18:07着（来居・いそかぜ）の3経路が表示され、「おすすめ」「時間が短い」「料金が安い」「乗換が少ない」の全4モードで維持された。console error/warningは0件だったため解決済みとする。
...
productionアセットを同期したdebug署名APKをPixel 9 / Android 16（API 36）エミュレータへ導入し、履歴ありでは乗換案内から運航状況へ戻ること、英語トップではLauncherへ終了することを確認した。`npm run test`は95 files・903 passed・1 skipped、`npm run lint`は0 errors・既存warning 66件。

2026年7月25日、moto g05 / Android 15のGoogle Play内部テスト版v2.4（24001）で、履歴がある画面では前画面へ戻り、時刻表ルートではMotorola Launcherへ終了することを再確認した。最終実機ログのクラッシュ・ANR・通信例外も0件だったため解決済みとする。
...
productionアセット入りdebug署名APKを上書き導入し、Pixel 9 / Android 16（API 36）でオンラインの定期運航・波高1.0mを取得後、機内モードでプロセスを再起動した。修正版では同じ運航状況と更新時刻が表示された。通信復元と更新インストール後の英語設定保持も確認した。`npm run test`は95 files・905 passed・1 skipped、`npm run lint`は0 errors・既存warning 66件。

2026年7月25日、moto g05 / Android 15のGoogle Play内部テスト版v2.4（24001）で、オンライン取得後にWi-Fiとモバイル通信を切り、アプリを完全終了・再起動しても更新時刻、定期運航、波高1.0mが表示されることを確認した。Wi-Fi復帰後も正常に再表示され、最終ログのクラッシュ・ANR・SSL・通信例外は0件だったため解決済みとする。
