...
```json
{
  ...
  "UI": {
    "TRANSPORT_FILTER": "交通手段",
    "SEARCH_LOCATION_PLACEHOLDER": "港・停留所・空港を検索",
    "PREPARING": "準備中"
  },
  "SEGMENT": {
    "PLATFORM": "のりば",
...
2. **翻訳の追加**
   ...
   - 未翻訳のキーは日本語を表示
   - 交通手段/地点タイプは `TRANSPORT_MODES` / `LOCATION_TYPES` を優先使用
...
- **地点名の取得**: `LocationType` に応じて `PORTS` / `STOPS` / `AIRPORTS` を参照する
- **事業者名・路線名**: `operatorId` / `serviceId` が翻訳キーに一致する場合のみ翻訳し、なければ原文を表示
- **便名/便番号**: 航空便番号などは翻訳せず原文表示を基本とする
